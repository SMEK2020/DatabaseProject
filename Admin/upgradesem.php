...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($session) && !empty($semid)) {
        // Prepare SQL statement to fetch semid for the selected session
        $sql = "SELECT semid FROM semester WHERE session = ? LIMIT 1";
        $stmt = $conn->prepare($sql);
        ...
        if ($result->num_rows > 0) {
            // Update semid in the semester table
            $sql_update = "UPDATE semester SET semid = ? WHERE session = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("is", $semid, $session);

            if ($stmt_update->execute()) {
                $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                                Semid updated successfully!
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>';
            } else {
                $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                                Error updating semid: ' . $stmt_update->error . '
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>';
            }
            $stmt_update->close();
            ...
            $message = '<div class="alert alert-warning alert-dismissible fade show" role="alert">
                            No semester found for the selected session.
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>';
        }
        $stmt->close();
...
<div class="card shadow p-4">
    <h3 class="text-center mb-4">Update Semid for Selected Session</h3>
    <form method="post">
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="semid" class="form-label">Semid:</label>
            <input type="number" name="semid" id="semid" class="form-control" required>
        </div>
        <button type="submit" class="btn btn-primary w-100">Update Semid</button>
    </form>
</div>
        ...
        <div class="col-md-4">
            ...
            <a href="#" class="btn btn-secondary m-3">Course Enroll</a>
            <a href="#" class="btn btn-primary m-3">Upgrade Semester</a>
